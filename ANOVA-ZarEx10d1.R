...
#    format for data. 
#
# Library plyr provides tools for splitting, applying and combining data.
#  The R routine raply() replicates an expression and returns results in an array.
#  For example, the command out = raply(.n=length(X1),.exp='group1')
#  writes n=5 times the expression group1.
#  After the command 
#     write.table(X1,file='DummyFile.txt',row.names=out,col.names=NA)
#  the file DummyFile.txt reads:
#  "" "x"
# "group1" 60.8
# "group1" 67
# "group1" 65
# "group1" 68.6
# "group1" 61.7
#  The command out = raply(.n=length(X2),.exp='group2')
#  writes n=5 times the expression group2.
#  After the command
#    write.table(X2,file='DummyFile.txt',append=T,row.names=out,col.names=F)
# (note the argument append=T for appending addition text to an already existing file)
# the file DummyFile.txt reads:
# "" "x"
# "group1" 60.8
# "group1" 67
...
# "group2" 71.8
#
# Data for further groups are added in the same way.
#
