print('file: EIVnumEx1.R')
print(date())
# Errors in variables (EIV): artificial data, small sample size
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
  ...
  n = 50    # sample size; large (1e3) or small (50)
  sigxError = 0.5
  sigyError = 14*sigxError # sigyError/sigxError close, however, not equal to SlopeTrue
  # (1a) xi values (true) from a normal distribution:
  ...
  x = xiTrue+rnorm(n,0,sigxError)   # 'observations'
  y = yTrue+rnorm(n,0,sigyError)    # 'observations'
  ...
  # library(latex2exp)
  sflag = 1
  if (sflag == 1) { # n=50 and true lambda gives good MLE results
    # png('EIVnutrientMC230127n50.png',width=16,height=16,units='cm',res=300)
