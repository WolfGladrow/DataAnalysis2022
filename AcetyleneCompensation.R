...
sflag = 11
if (sflag == 11) {
  # png('AcetyleneCompensation170626.png',width=16,height=16,units='cm',res=300)
  plot(x=NA,y=NA,type='l',lwd=3,col='blue',xlab='',ylab='j',las=1,
       ylim=c(0,NoP+2),xlim=c(-11,11),cex.lab=1.5)
  yp = 0; 
  for(j in 1:NoP) {xa = c(0,j); xa[1] = betaEst1[j]*X[1,j]; 
  arrows(0,j,xa[1],xa[2],col='black',code=2,angle=10,length=0.1,lwd=3); 
  yp = yp+xa[1]}
  arrows(0,NoP+1,yp,NoP+1,col='magenta',code=2,angle=30,lwd=5,length=0.01)
