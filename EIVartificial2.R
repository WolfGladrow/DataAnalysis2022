...
# -----------------------------------------------------------
  ...
  lambdaTrue = (sigx/sigy)^2 # ratio of error variances (def. according to
                             # Casella & Berger, 2002)
  # (1a) true xi (<-> P) values from a normal distribution:
  # ximean = 1.9    # mean x concentration
  # xisigma = 0.7   # standard deviation
  # xi = rnorm(n,ximean,xisigma)
  dxi = (3.3-0.5)/(n-1) # equidistant 
  xi = seq(0.5,3.3,dxi)
  # (1b) true zeta (<-> N) values
  ...
  xc = mean(x); yc = mean(y)
  # line with slope SlopeGeo through centroid: y-yc = SlopeGeo*(x-xc)
...
# -----------------------------------------------------------
  ...
  sflag = 2
  if (sflag == 2) {
  # png('SlopeLambda230206b.png',width=16,height=16,units='cm',res=300)
  ...
  text(0.13,SlopeMLE-0.5,TeX('$MLE\\, (true\\, \\lambda)$'),col='magenta',pos=4)
