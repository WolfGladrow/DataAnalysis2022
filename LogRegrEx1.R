...
write.table(x,'xLogReg160930.txt')
write.table(y,'yLogReg160930.txt')
}
if (dflag == 2) { # read data from file
xread = read.table('xLogReg160930.txt',header=T); x = xread$x
yread = read.table('yLogReg160930.txt',header=T); y = yread$x # note: $x!!!
}
# logistic regression:
...
sflag = 2
if(sflag == 1) {
  # png('LogResData160930.png',width=16,height=16,units='cm',res=300)
