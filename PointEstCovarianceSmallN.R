print('file: PointEstCovarianceSmallN.R')
# covariance & correlation estimators: small n
set.seed(1953)
...
corEst = numeric(M)
for(k in 1:M) {
  q=sampleUcorFct(r,n); x=q[1:n]; y=q[(n+1):(2*n)]; 
  covEst[k] = cov(x,y); corEst[k] = cor(x,y)
}
sflag = 1
if (sflag == 1) {
