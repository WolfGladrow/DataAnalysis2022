...
t0 = t.test(y1,y2)$statistic; print(c(round(t0,4),'t0'))
tmax=4; dx=0.001; xa=seq(-tmax,tmax,dx); ya=dt(xa,nu);
#   tmax=4; xa=-tmax:0.001:tmax; ya=tpdf(xa,nu);
# % display('Scales of evidence')
...
u1=-tmax; u2=ts; v1=dt(u1,nu); v2=dt(u2,nu); du=(u2-u1)/50; 
un=seq(u1,u2,du); vn=dt(un,nu); uf=c(u2,u1,un); vf=c(0,0,vn); 
# png('Ott01Ex6d3rr140907.png',width=16,height=12,units='cm',res=300)
...
lines(xpz1,ypz1,col='magenta',lty=2)
xt = -3;
text(xt,0.46,TeX('$-t_{0.05(1)22} = 1.7174$'),col='blue',cex=1.5,pos=4)
text(xt,0.36,TeX('$\\alpha = 0.05$'),col='blue',cex=1.5,pos=4)
text(-3.9,0.40,TeX('$t_s = -2.2709$'),col='magenta',cex=1.5,pos=4)
text(-0.5,0.41,TeX('$magenta\\, area = 0.0166$'),col='magenta',cex=1.5,pos=4)
text(-tmax,0.18,TeX('$p = 0.0166$'),col='magenta',cex=1.5,pos=4)
polygon(uf,vf,col='magenta')
# dev.off()