...
library(latex2exp)
# png('tentA160722.png',width=16,height=16,units='cm',res=300)
...
lines(y1,q1,type='l',lwd=4,col='black',xlab='y',ylab='q(y)')
lines(y2,q2,type='l',lwd=4,col='magenta')
# subplot 2:
plot(1,type='n',xlim=c(-0.1,1.1),ylim=c(0,1.1),xlab='y',ylab='F(y)',las=1,cex.lab=1.5)
lines(y1,F1,type='l',lwd=4,col='black',xlab='y',ylab='F(y)')
lines(y2,F2,type='l',lwd=4,col='magenta')
# subplot 3:
plot(1,type='n',xlim=c(-0.1,1.1),ylim=c(-0.5,1.5),xlab='x',ylab=NA,las=1,cex.lab=1.5)
title(ylab=TeX('$F^{-1}(x)$'),line=2.3,cex.lab=1.5)
lines(x1,y1x,type='l',lwd=4,col='black')
lines(x2,y2xm,type='l',lwd=4,col='magenta')
lines(x2,y2xp,type='l',col='green',pch=22,lty=2)
lines(x1,-y1x,type='l',col='green',pch=22,lty=4)
# dev.off()